...
const Fonst = {
  ...
  */
  ...
  _toggle(directory) {
    return { ...directory,
      isExpanded: !directory.isExpanded
    };
  },
  ...
  */
  async getDB() {
    ...
    try {
      ...
      //initialize database.
      await this.initDb();
      let database = Object.keys(_SCHEMA).map(key => {
        return {
          key,
          value: _SCHEMA[key]
        };
  ...
  async updateProjectOpenFiles({
    ...
    for (let filename of Object.keys(openFiles)) {
      ...
      openFiles[filename] = { ...openFiles[filename],
        sourceCode: updatedOpenFiles[filename].sourceCode
      };
    ...
    try {
      ...
      console.log(" unable to update currentFile object ", e);
    }
  ...
  */
  async newDirectory(filepath, children, project_type, mappingDir = false) {
    ...
    updatedValues = mappingDir ? updatedValues : updatedValues.concat({
      key: "appState",
      value: A.DIR_OPEN
  ...
  async updateProjectDependencies(dependencies) {
    ...
    return this.setValue('project', { ...project,
      dependencies
    });
  },
  ...
  async mutateCurrentDir({
    ...
    let updatedDirectory = _mutate(currentDirectory); //if updatedDirectory is valid then update value of currentDirectory key.
  ...
  async toggleFolder(folderpath) {
    ...
    try {
      currentDirectory = await this.getValue('currentDirectory');
    } catch (e) {} //make appropriate manipulations to the current directory.
  ...
  async closeFile({
    ...
    let currentFile = {}; //console.log(' open files ',openFiles);
      ...
      switch (oparray.length) {
        case 1:
          ...
          try {
            ...
            await this.mutateCurrentDir({
              _mutate
            });
          } catch (e) {}

          openFiles = {}; //updated application state
        ...
        default:
          ...
          delete openFiles[filename]; //update value of new currentFile.
          //new current file values.

          currentFile = openFiles[updatedFile]; //console.log(' new current file ',currentFile);

          updatedValues = updatedValues.concat({
            key: 'currentFile',
            value: currentFile
          });
          openFiles[updatedFile] = { ...currentFile,
            isCurrent: true
          };
          break;
      }
    } else
      /*if file is not current then we delet file.*/
      delete openFiles[filename]; //update keys.
  ...
  async newNode({
    ...
    if (nodetype == 'file') {
      let fileType = path.extname(nodename),
          lang;
    ...
    try {
      ...
      currentDirectory = await this.getValue('currentDirectory'); //change directory add new file to directory whose path matches that given

      currentDirectory = await DirFunctions.changeDir(currentDirectory, nodepath.split('/'), _change); //console.log(" updated directory with new node ",currentDirectory);

      return this.setValue('currentDirectory', currentDirectory);
    } catch (e) {
      /* unable to mutate current directory. */
  ...
  async mutateOpenFiles({
    ...
    try {
      ...
      console.log(" error obtained in mutateOpenFiles function ", e);
  ...
  async openFile({
    ...
    let openFilesActions = []; //obtain the current filename for the directory.

    let filename = parts[parts.length - 1];
    let currentFile = this.curFileFormat({
      filename,
      sourceCode,
      filepath
    ...
    try {
      if (newlyCreated) {
        ...
        function _change(directory) {
          let children = directory.children;
          children[filename] = { ...children[filename],
            newlyCreated: false
          };
          return { ...directory,
            children
        ...
        currentDirectory = DirFunctions.changeDir(currentDirectory, parts.filter(f => f != filename), _change); //set current directory value to false.
  ...
  async saveFile() {
    ...
    } catch (e) {} //set values of currentDirectory,currentFile and openFiles.
  ...
  async closeCurrentDirectory() {
    let currentValues = [{
      key: "currentDirectory",
      value: _SCHEMA.currentDirectory
    }, {
      key: "appState",
      value: null
    }, {
      key: "currentFile",
      value: _SCHEMA.currentFile
    }]; //set db values.
