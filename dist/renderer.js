...
const functionality = {
  openDirectory(projecttype) {
    ...
    return ipcRenderer.invoke(E.OPEN_DIR_PROMPT, projecttype).then(data => {
      let {
        canceled,
        filePaths
      } = data; //if user canceled open directory prompt return.
  ...
  async openFile(filepath, prevOpenFilePath, newlyCreated = false) {
    ...
    let code = await ipcRenderer.invoke(E.OPEN_FILE, {
      filepath,
      newlyCreated
    }); //once source code is found.

    return $funcs.openFile({
      filepath,
      sourceCode: code,
      prevOpenFilePath,
      newlyCreated
  ...
  async showFile(object) {
    return $funcs.openFile(object);
  },
  ...
  runProjectRendering({
    ...
    return ipcRenderer.send(E.RUN_GUI, {
      filepath,
      project: projecttype,
      sourceCode
    });
  ...
  stopRenderering() {
    ...
    GUIUpdater.stopMonitoring(); //stop the current rendering process which is running.
  ...
  createNode(nodename, type) {
    let {
      nodepath
    } = ApplicationState.session.newNode; //send absolute path of file to main process.
